...
/**
 * Convert normalized mobile number (07XXXXXXXX) to international format for external APIs.
 * Landlines are not valid for Paystack/SMS; use only for mobile numbers.
 *
 * @param phone - Already normalized 07XXXXXXXX
 * @returns 2547XXXXXXXX (no + prefix)
 * @throws Error if not 07XXXXXXXX
 */
export function toInternationalFormat(phone: string): string {
  if (!phone || !/^07\d{8}$/.test(phone)) {
    throw new Error(
      `toInternationalFormat requires normalized mobile 07XXXXXXXX. Received: ${phone}. Landlines are not valid for this API.`
    );
  }
