...
export type OutboundPayload = Record<string, unknown> & {
  ...
  customerId?: string;
  /** Whether this message was generated by a scheduled/system process. This is metadata;
   * the actual deferral behaviour is controlled by the trigger's dispatchPolicy. */
    ...
    if (config.channels.whatsapp) {
      ...
      if (whatsappBody && recipients.length > 0) {
        for (const { phone, userId } of recipients) {
          ...
          const deferWhatsApp = config.dispatchPolicy?.whatsapp?.deferOutsideWindow ?? false;
          if (deferWhatsApp) {
            await this.notificationSchedulingService.deferOrSendWhatsApp(ctx, {
              channelId: config.audience === 'platform_admin' ? undefined : channelId,
              triggerKey,
              recipient: phone,
              body: whatsappBody,
