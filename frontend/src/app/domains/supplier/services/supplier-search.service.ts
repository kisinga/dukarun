...
import { GET_SUPPLIERS } from '../operations.graphql';
import { GET_CUSTOMERS } from '@dukarun/customer';
...
export class SupplierSearchService implements CacheSyncEntityHandler {
  ...
  /**
   * Fetch purchase summary for a supplier preview: total purchases and last purchase reference.
   */
  async getSupplierPurchasePreview(
    supplierId: string,
  ): Promise<{ total: number; lastRef: string } | null> {
    try {
      const client = this.apolloService.getClient();
      const result = await client.query({
      ...
      return { total, lastRef };
    } catch (err) {
      console.warn('[SupplierSearch] getSupplierPurchasePreview failed', { supplierId }, err);
      return null;
    }
  }
