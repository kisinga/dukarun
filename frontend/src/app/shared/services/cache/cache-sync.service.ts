...
import type { CacheSyncEntityHandler, CacheSyncEntityType } from './cache-sync-handler.interface';
...
export class CacheSyncService {
  ...
  constructor() {
    effect(() => {
      const channelId = this.companyContext.activeCompanyId();
      if (channelId) {
  ...
  private connect(channelId: string): void {
    ...
    const channelToken = this.companyContext.getChannelToken();
    ...
    this.eventSource.onmessage = (e: MessageEvent) => {
      setTimeout(() => {
        try {
          ...
          if (msg.channelId !== this.companyContext.activeCompanyId()) return;
  ...
  private scheduleReconnect(channelId: string): void {
    ...
    this.reconnectTimeout = setTimeout(() => {
      this.reconnectTimeout = null;
      if (this.companyContext.activeCompanyId() === channelId) {
        this.connect(channelId);
      }
    }, delay);
