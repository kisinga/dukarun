...
if (ci === -1) { console.error('usage: node scripts/etl/verify.mjs --channel <id>'); process.exit(1); }
...
if (!chan) { console.error(`channel ${CHANNEL_ID} not found`); process.exit(1); }
const mapRow = await T(
  `select new_id from public.etl_id_map where old_type='channel' and old_id=$1`, [String(CHANNEL_ID)]);
if (!mapRow.length) { console.error(`channel ${CHANNEL_ID} not migrated (no etl_id_map entry)`); process.exit(1); }
...
const idMap = await T('select old_type, old_id, new_id from public.etl_id_map where company_id=$1', [companyId]);
const maps = {};
...
async function countCheck(label, srcCount, tgtCount, note = '') {
  ok(label, srcCount === tgtCount, `src=${srcCount} tgt=${tgtCount}${note ? ' (' + note + ')' : ''}`);
}
...
const srcCustomers = await S(
  `with ids as (
   ...
   ) select count(*)::int n from customer c join ids on ids.id=c.id where c."deletedAt" is null`, [CHANNEL_ID]);
const tgtCustomers = await T('select count(*)::int n from public.customers where company_id=$1', [companyId]);
await countCheck('customers', srcCustomers[0].n, tgtCustomers[0].n,
  'channel members + order/ledger-referenced, excl. soft-deleted');

const srcProducts = await S(
  `select count(*)::int n from product p join product_channels_channel pcc on pcc."productId"=p.id
   where pcc."channelId"=$1 and p."deletedAt" is null`, [CHANNEL_ID]);
const tgtProducts = await T('select count(*)::int n from public.products where company_id=$1', [companyId]);
await countCheck('products', srcProducts[0].n, tgtProducts[0].n, 'excl. soft-deleted');

const srcVariants = await S(
  `select count(*)::int n from product_variant v join product_variant_channels_channel vcc on vcc."productVariantId"=v.id
   where vcc."channelId"=$1 and v."deletedAt" is null`, [CHANNEL_ID]);
const tgtVariants = await T('select count(*)::int n from public.product_variants where company_id=$1', [companyId]);
await countCheck('variants', srcVariants[0].n, tgtVariants[0].n);

const srcBatches = await S(
  'select count(*)::int n from inventory_batch where "channelId"=$1 and quantity>0', [CHANNEL_ID]);
const tgtBatches = await T('select count(*)::int n from public.inventory_batches where company_id=$1', [companyId]);
await countCheck('inventory_batches', srcBatches[0].n, tgtBatches[0].n, 'only quantity>0 migrated');

// orders: completed + voided are migrated; drafts/parked are skipped by design
const srcOrders = await S(
  `select o.id, o.state, o."customFieldsReversedat" from "order" o
   join order_channels_channel occ on occ."orderId"=o.id where occ."channelId"=$1 and o.type='Regular'`, [CHANNEL_ID]);
const COMPLETED = ['Fulfilled', 'Shipped', 'Delivered', 'PartiallyShipped', 'PaymentSettled'];
const migratable = srcOrders.filter(o =>
  o.customFieldsReversedat != null || o.state === 'Cancelled' || COMPLETED.includes(o.state));
const parked = srcOrders.filter(o => !migratable.includes(o));
const tgtOrders = await T('select count(*)::int n from public.orders where company_id=$1', [companyId]);
await countCheck('orders', migratable.length, tgtOrders[0].n,
  `${parked.length} draft/parked deliberately skipped`);
for (const o of parked) notes.push(`skipped draft/parked order ${o.id} (${o.state}) — close before cutover`);
...
const tgtLines = await T('select count(*)::int n from public.order_lines where company_id=$1', [companyId]);
await countCheck('order_lines', srcLines[0].n, tgtLines[0].n, 'voided orders use orderPlacedQuantity');

const srcPays = await S(
  'select count(*)::int n from payment where "orderId" = any($1::int[]) and amount > 0',
  [migratable.map(o => o.id)]);
const tgtPays = await T('select count(*)::int n from public.payments where company_id=$1', [companyId]);
await countCheck('payments', srcPays[0].n, tgtPays[0].n);

const srcEntries = await S('select count(*)::int n from ledger_journal_entry where "channelId"=$1', [CHANNEL_ID]);
const tgtEntries = await T('select count(*)::int n from public.ledger_journal_entries where company_id=$1', [companyId]);
await countCheck('ledger_journal_entries', srcEntries[0].n, tgtEntries[0].n, 'verbatim');

const srcJLines = await S('select count(*)::int n from ledger_journal_line where "channelId"=$1', [CHANNEL_ID]);
const srcDouble = await S(
  'select count(*)::int n from ledger_journal_line where "channelId"=$1 and debit>0 and credit>0', [CHANNEL_ID]);
const tgtJLines = await T('select count(*)::int n from public.ledger_journal_lines where company_id=$1', [companyId]);
await countCheck('ledger_journal_lines', srcJLines[0].n + srcDouble[0].n, tgtJLines[0].n,
  `verbatim; ${srcDouble[0].n} double-sided source line(s) split into two single-sided (target check constraint)`);

const srcSessions = await S('select count(*)::int n from cashier_session where "channelId"=$1', [CHANNEL_ID]);
const tgtSessions = await T('select count(*)::int n from public.cashier_sessions where company_id=$1', [companyId]);
...
const srcCounts = await S(
  `select count(*)::int n from cash_drawer_count where "channelId"=$1
     and lower("countType") in ('opening','closing','mid_shift')`, [CHANNEL_ID]);
const tgtCounts = await T('select count(*)::int n from public.cash_drawer_counts where company_id=$1', [companyId]);
await countCheck('cash_drawer_counts', srcCounts[0].n, tgtCounts[0].n);

const srcRecons = await S('select count(*)::int n from reconciliation where "channelId"=$1', [CHANNEL_ID]);
const tgtRecons = await T('select count(*)::int n from public.reconciliations where company_id=$1', [companyId]);
...
const srcReconAccts = await S(
  `select count(*)::int n from reconciliation_account ra join reconciliation r on r.id=ra."reconciliationId"
   where r."channelId"=$1`, [CHANNEL_ID]);
...
const tgtAccts = await T('select count(*)::int n from public.ledger_accounts where company_id=$1', [companyId]);
await countCheck('ledger_accounts (provisioned)', 21, tgtAccts[0].n);
const tgtPms = await T('select count(*)::int n from public.payment_methods where company_id=$1', [companyId]);
await countCheck('payment_methods (provisioned)', 4, tgtPms[0].n);
const tgtLoc = await T('select count(*)::int n from public.stock_locations where company_id=$1', [companyId]);
...
const srcTot = await S(
  `select coalesce(sum(debit),0)::text d, coalesce(sum(credit),0)::text c
   from ledger_journal_line where "channelId"=$1`, [CHANNEL_ID]);
const tgtTot = await T(
  `select coalesce(sum(debit),0)::text d, coalesce(sum(credit),0)::text c
   from public.ledger_journal_lines where company_id=$1`, [companyId]);
ok('Σdebit = Σcredit (source)', Number(srcTot[0].d) === Number(srcTot[0].c),
  `${srcTot[0].d} vs ${srcTot[0].c}`);
ok('Σdebit = Σcredit (target)', Number(tgtTot[0].d) === Number(tgtTot[0].c),
  `${tgtTot[0].d} vs ${tgtTot[0].c}`);
ok('ledger totals match src=tgt', Number(srcTot[0].d) === Number(tgtTot[0].d) && Number(srcTot[0].c) === Number(tgtTot[0].c),
...
ok('closing balance per account code (exact)', balMismatch === 0,
...
const srcStock = await S(
  `select b."productVariantId" vid, v.sku, sum(b.quantity) qty
   ...
   where b."channelId"=$1 and b.quantity>0 group by 1,2 order by v.sku`, [CHANNEL_ID]);
const tgtStockRows = await T(
  `select v.id, sum(b.remaining) rem from public.product_variants v
   ...
   where v.company_id=$1 group by v.id`, [companyId]);
...
for (const r of srcStock) {
  ...
  console.log(`  ${good ? 'PASS' : 'FAIL'}  variant ${r.sku}: src qty=${r.qty} tgt remaining=${t ?? '(variant missing)'}`);
  if (!good) failures++;
}
...
for (const r of srcAr) {
  ...
  const name = newCid
    ? (await T('select first_name || \' \' || coalesce(last_name,\'\') n from public.customers where id=$1', [newCid]))[0]?.n
    : null;
  const t = newCid ? tgtAr.get(newCid) ?? 0 : null;
  const good = t !== null && Number(r.bal) === t;
  if (!good) arMismatch++;
  console.log(`  ${good ? 'PASS' : 'FAIL'}  customer ${r.cid}${name ? ` (${name.trim()})` : ''}: src AR=${r.bal} tgt AR=${t ?? '(unmapped)'}`);
  if (!good) failures++;
}
...
const noVariants = await S(
  `select p.id, t.name from product p join product_channels_channel pcc on pcc."productId"=p.id
   ...
   where pcc."channelId"=$1 and p."deletedAt" is null
     and not exists (select 1 from product_variant v where v."productId"=p.id and v."deletedAt" is null)`, [CHANNEL_ID]);
...
const noPrice = await S(
  `select v.id, v.sku from product_variant v join product_variant_channels_channel vcc on vcc."productVariantId"=v.id
   where vcc."channelId"=$1 and v."deletedAt" is null
     and not exists (select 1 from product_variant_price p where p."variantId"=v.id and p."channelId"=$1)`, [CHANNEL_ID]);
...
const payMismatch = await S(
  `select o.id, o.code, o."subTotalWithTax" total,
   ...
   where occ."channelId"=$1 and o.state='PaymentSettled'`, [CHANNEL_ID]);
for (const o of payMismatch) {
  if (Number(o.paid) !== Number(o.total)) {
    (Number(o.paid) === 0 || Number(o.paid) < Number(o.total))
      ? notes.push(`order ${o.id} (${o.code}): paid ${o.paid} of ${o.total} -> migrated as credit sale`)
...
const unmapped = await T(
  `select count(*)::int n from public.ledger_journal_lines
   where company_id=$1 and (meta->>'customerId' ~ '^[0-9]+$' or meta->>'supplierId' ~ '^[0-9]+$')`, [companyId]);
if (unmapped[0].n) warnLines.push(`${unmapped[0].n} target ledger line(s) kept an unmigrated int customerId/supplierId in meta`);
const unmappedOrders = await T(
  `select count(*)::int n from public.ledger_journal_lines
   where company_id=$1 and meta ? 'orderId'`, [companyId]);
if (unmappedOrders[0].n) warnLines.push(`${unmappedOrders[0].n} target ledger line(s) kept meta.orderId (order skipped as draft/parked)`);
