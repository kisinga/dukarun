...
const { rows: chRows } = await src.query('select id, code from channel where id=$1', [FLAGS.channel]);
...
if (!companyId) {
  ...
  if (companyId) console.log(`  WARN: no etl_id_map channel row; fell back to companies.code '${channel.code}'`);
}
if (!companyId) {
...
const STEPS = [
  ...
  ['storage.objects (product-images prefix)',
    "delete from storage.objects where bucket_id='product-images' and name like $1 || '/%'"],
...
try {
  ...
  const { rows: mapped } = await tgt.query(
    `select new_id from public.etl_id_map where old_type='user' and company_id=$1`, [companyId]);
