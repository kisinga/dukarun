...
1. **Missing COGS on cashier-flow sales** — the parked→settled sale posted revenue but no
   ...
   new system's atomic posting is strictly stronger. _(If the worker is required, production
   gets COGS late; if it's the skip path, production never gets it for these sales.)_
...
- The archived backend needs the TIE-OUT patch in
  `archive/vendure/backend/src/plugins/ledger/ledger.plugin.ts`
