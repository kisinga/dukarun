 ...
 *
 * Uses only persisted DB columns: Order.totalWithTax is @Calculated (use subTotalWithTax + shippingWithTax);
 * OrderLine.linePriceWithTax is @Calculated (use listPrice * quantity). Custom fields: see vendure-config customFields.
 */
export class CreateAnalyticsMaterializedViews9300000000000 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    // Drop existing views so we always apply the correct definition (idempotent, recovers from partial runs)
    ...
    // 2. Daily order stats (Order.totalWithTax is @Calculated; use persisted subTotalWithTax + shippingWithTax)
    await queryRunner.query(`
      ...
      SELECT
        ...
        SUM(o."subTotalWithTax" + COALESCE(o."shippingWithTax", 0))   AS total_revenue,
        AVG(o."subTotalWithTax" + COALESCE(o."shippingWithTax", 0))   AS avg_order_value,
