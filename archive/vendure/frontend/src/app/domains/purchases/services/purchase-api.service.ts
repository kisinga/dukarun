...
import { RECORD_PURCHASE } from '../operations.graphql';
...
export class PurchaseApiService {
  ...
  async recordPurchase(draft: PurchaseDraft, options?: { saveAsDraft?: boolean }): Promise<any> {
    ...
    if (!options?.saveAsDraft && draft.paymentStatus !== 'pending') {
      const debitAccountCode = draft.paymentAccountCode?.trim();
      if (!debitAccountCode) {
        throw new Error('Payment source account is required');
      }
      input.payment = {
        amount:
          draft.paymentAmount != null ? Math.round(draft.paymentAmount * 100) : totalCostCents,
        debitAccountCode,
        reference: draft.paymentReference?.trim() || undefined,
      };
    }
