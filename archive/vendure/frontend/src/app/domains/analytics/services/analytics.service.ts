...
import type { GetAnalyticsStatsQuery } from '../../../shared/graphql/generated/graphql';
import { GET_ANALYTICS_STATS } from '../operations.graphql';
import { ApolloService } from '../../../shared/services/apollo.service';
import type { AnalyticsPeriod } from '../../../shared/components/dashboard/charts';
...
/** Cache TTL in ms; skip refetch if same period was fetched within this window */
...
export class AnalyticsService {
  ...
  private lastFetchedPeriod: AnalyticsPeriod | null = null;
  ...
  async fetch(period: AnalyticsPeriod, limit = 10): Promise<void> {
    const now = Date.now();
    if (
      this.statsSignal() != null &&
      this.lastFetchedPeriod === period &&
      now - this.lastFetchedAt < ANALYTICS_CACHE_TTL_MS
    ) {
      return;
    }
