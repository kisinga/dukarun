    ...
    <div class="grid gap-4 xl:grid-cols-5">
      <section class="rounded-box border border-base-300 bg-base-100 p-4 xl:col-span-3">
        <header class="flex flex-wrap items-start justify-between gap-3">
          <div>
            <h4 class="font-semibold">Daily demand</h4>
        ...
        @if (hasDemand()) {
          <div class="mt-4 flex flex-wrap items-center gap-4 text-xs text-base-content/60">
            ...
            <span class="ml-auto">Peak {{ peakDemandLabel() }} units</span>
          </div>
          <div
            class="relative mt-3 h-52 overflow-x-auto overflow-y-hidden border-y border-base-300/70 bg-base-200/20"
            role="img"
          ...
          >
            ...
            >
              @for (point of demandPoints(); track point.day; let index = $index) {
                <div
                  class="relative flex h-full min-w-1 flex-1 items-end justify-center gap-px"
                ...
                >
                  ...
                  @if (point.returns > 0) {
                    <span
                      class="w-1/3 min-w-1 rounded-t-field bg-error/70"
                      [style.height.%]="point.returnHeight"
                    ></span>
                  }
                  @if (showAxisLabel(index, demandPoints().length)) {
                    <span class="absolute -bottom-6 whitespace-nowrap text-xs text-base-content/55">
                      {{ shortDay(point.day) }}
                    </span>
                  }
                </div>
      ...
      <section class="rounded-box border border-base-300 bg-base-100 p-4 xl:col-span-2">
        <header class="flex flex-wrap items-start justify-between gap-3">
          <div>
            ...
            <p class="type-caption mt-1">Estimated closing stock across the period.</p>
          </div>
...
export class ProductActivityChartComponent {
  ...
  protected readonly demandPoints = computed(() => {
    const source = this.trend().map(point => ({
      day: point.day,
  ...
  protected readonly hasDemand = computed(() =>
    this.demandPoints().some(point => point.gross > 0 || point.returns > 0 || point.net > 0)
  );
  protected readonly activeDays = computed(
    () => this.demandPoints().filter(point => point.net !== 0 || point.returns > 0).length
  );
  protected readonly totalNet = computed(() =>
    this.demandPoints().reduce((sum, point) => sum + point.net, 0)
  );
  protected readonly totalGross = computed(() =>
    this.demandPoints().reduce((sum, point) => sum + point.gross, 0)
  );
  protected readonly totalReturns = computed(() =>
    this.demandPoints().reduce((sum, point) => sum + point.returns, 0)
  );
  ...
  protected readonly chartWidth = computed(() => Math.max(360, this.demandPoints().length * 12));
  protected readonly demandAriaLabel = computed(
    () =>
      `Daily demand: ${this.totalNetLabel()} net units across ${this.activeDays()} active days, ${this.returnRateLabel()} returned`
  );
  ...
  protected readonly stockPoints = computed(() => {
    ...
    const known = source
      ...
      .filter((point): point is { day: string; quantity: number; index: number } =>
  ...
  protected readonly currentStockLabel = computed(() =>
    this.quantity(this.stockPoints().at(-1)?.quantity ?? 0)
  );
  ...
  protected readonly maximumStockLabel = computed(() =>
    this.quantity(Math.max(...this.stockPoints().map(point => point.quantity), 0))
  );
  protected readonly stockoutDays = computed(
    () => this.stockPoints().filter(point => point.quantity <= 0).length
  );
  protected readonly firstStockDay = computed(() =>
    this.shortDay(this.stockPoints()[0]?.day ?? '')
  );
  protected readonly lastStockDay = computed(() =>
    this.shortDay(this.stockPoints().at(-1)?.day ?? '')
  );
  ...
  protected demandPointLabel(point: {
    ...
    return `${this.shortDay(point.day)}: ${this.quantity(point.net)} net sold, ${this.quantity(point.returns)} returned`;
  }
