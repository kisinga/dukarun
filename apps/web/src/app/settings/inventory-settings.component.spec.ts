...
describe('InventorySettingsComponent', () => {
  async function render(overrides: Partial<CompanySettings> = {}) {
    ...
    const fixture = TestBed.createComponent(InventorySettingsComponent);
  ...
  it('loads and owns inventory controls', async () => {
    const { fixture, settingsService } = await render({ batch_expiry_enabled: false });
    const root = fixture.nativeElement as HTMLElement;

    expect(settingsService.getSettings).toHaveBeenCalledOnce();
    expect(root.textContent).toContain('Low-stock threshold');
    expect(root.textContent).toContain('Expiry fields are hidden');
  });
