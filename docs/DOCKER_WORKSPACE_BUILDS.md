...
This document describes how Docker images for **backend**, **frontend**, and **super-admin** are built and why the build context is the repository root.
...
- **Dockerfile:** `backend/Dockerfile`, `frontend/Dockerfile`, or `super-admin/Dockerfile`
...
The repo is an **npm workspace** (root `package.json` has `"workspaces": ["backend", "frontend", "super-admin"]`). There is a **single** `package-lock.json` at the root that locks all workspace dependencies.
...
- Do **not** rely on running `npm i` only inside `frontend/` (or `backend/` or `super-admin/`) to fix CI or Docker: from inside a workspace, npm still uses the root lockfile and may not change it in the way a root-only install would. For Docker/CI, the fix is to use root context and root lockfile (this pattern).
...
| **Build in Dockerfile** | `npm run build -w @dukarun/backend` (or frontend / super-admin) |
